...
class RegionAttention(nn.Module):
    ...
    def __init__(self, input_dim: int = 32, num_heads: int = 8) -> None:
        super().__init__()
    ...
    def forward(self, region_nodes: torch.Tensor) -> torch.Tensor:
        """Aggregate nodes within one facial region.

        Args:
            region_nodes: Tensor of shape (batch, T, num_nodes, D).

        Returns:
            Aggregated tensor of shape (batch, T, D).
        """
...
def weight_init(module: nn.Module) -> None:
    ...
    This function intentionally preserves the original code structure.
    """
...
class CustomFeedForward(nn.Module):
    """Feed-forward block used in the decoder."""
...
class CustomTransformerDecoderLayer(nn.Module):
    """Custom decoder layer used for visual reconstruction."""
...
class AttentionDecoder(nn.Module):
    """Decoder that reconstructs visual features from selected tokens."""
    ...
    def __init__(
        ...
        self.tgt_projection_visual = nn.Linear(165, d_model)
        self.linear_visual = nn.Linear(d_model, 165)
    ...
    def forward(
        ...
        return self.linear_visual(output)
...
class Multi_CrossAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        attn_output_xy = attn_output_xy.transpose(1, 2).contiguous().view(
            batch_size, -1, self.all_head_size
        )
        attn_output_yx = attn_output_yx.transpose(1, 2).contiguous().view(
            batch_size, -1, self.all_head_size
        )
...
class Net(nn.Module):
    """BHDD model for multimodal depression detection."""

    def __init__(self, adj_matrix: torch.Tensor, K: int = 186) -> None:
        super().__init__()
        self.feature_proj = nn.Linear(2, 32)
        self.region_attn = RegionAttention(input_dim=32, num_heads=8)
        self.gcn = GCN(
            in_features=32,
            hidden_features=64,
            out_features=32,
        ...
        self.linear1 = nn.Linear(9 * 32, 256)
        ...
        self.mhca = Multi_CrossAttention(hidden_size=128, all_head_size=128, head_num=8)
        self.mhca1 = Multi_CrossAttention(hidden_size=256, all_head_size=256, head_num=8)
    ...
    def init_global_nodes(self, lm_data: torch.Tensor) -> torch.Tensor:
        """Append 9 global region nodes to the 68 original facial landmarks."""
        ...
        for i, (start, end) in enumerate(regions):
            ...
            aggregated = self.region_attn(region_data)
    ...
    def forward(
        self,
        lm: torch.Tensor,
        au_pose_gaze: torch.Tensor,
        audio: torch.Tensor,
    ) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        """Forward pass.

        Args:
            lm: Landmark tensor of shape (batch, 915, 136).
            au_pose_gaze: AU/pose/gaze tensor of shape (batch, 915, 29).
            audio: Audio tensor of shape (batch, 186, 128).

        Returns:
            Tuple containing:
                - logits
                - first-layer attention map
                - selected top-k indices
                - original visual tensor used by reconstruction loss
                - reconstructed visual tensor
        """
        visual = torch.cat([lm, au_pose_gaze], dim=-1)
        batch_size = lm.size(0)

        lm = lm.view(batch_size, 915, 68, 2)
        lm = self.feature_proj(lm)
        lm = self.init_global_nodes(lm)
        lm_gcn = self.gcn(lm)
        lm_gcn = lm_gcn[..., -9:, :]
        lm_gcn = lm_gcn.view(batch_size, 915, -1)
        ...
        output_first_layer, attention_map_first_layer = self.transformer.forward_first_layer(
            transformer_input
        )

        output_first_layer_btd = output_first_layer.permute(1, 0, 2)
        selected_tokens = self.token_selector(output_first_layer_btd, attention_map_first_layer)
        topk_indices = self.token_selector.last_topk_indices

        selected_tokens_kbd = selected_tokens.permute(1, 0, 2)
        final_transformer_output_kbd = self.transformer.forward_remaining_layers(selected_tokens_kbd)
        final_transformer_output = final_transformer_output_kbd.permute(1, 0, 2)
        ...
        return result, attention_map_first_layer, topk_indices, visual, visual_reconstructed
